...
var owf = function(wrapper,data){
	...
	this.getWidth = function(){
		return Math.floor(parseInt(this.wrapper.style('width'))) - Math.floor(parseInt(this.wrapper.style('padding-left')))  - Math.floor(parseInt(this.wrapper.style('padding-right'))) ;
	};
	this.width = this.getWidth();
	this.getHeight = function(){
		return (parent.width > 600) ? this.width*.4 : 300;
	}
	this.height = this.getHeight();
	this.getBarWidth = function(count){
		var w = ((parent.width - parent.padding.left - parent.padding.right) - parent.barPadding * count)/count;
		return (w >= 1) ? w : 1;
	};
	this.barWidth = null;
	this.wrapper.append('button').text('Cavity Nesters').attr('class','switch-cavity').style({
			'background-color': parent.colors.main,
			'color': '#fff'
		})
		.on('click',function(){
			parent.wrapper.selectAll('button').style('background-color','rgb(148, 148, 148)');
			parent.updateCavities();
			d3.select(this).style('background-color',parent.colors.main);
		});
	this.wrapper.append('button').text('Migratory').attr('class','switch-migration').style({
			'background-color': 'rgb(148, 148, 148)',
			'color': '#fff'
		})
		.on('click',function(){
			parent.wrapper.selectAll('button').style('background-color','rgb(148, 148, 148)');
			parent.updateMigration();
			d3.select(this).style('background-color',parent.colors.main);
		});
	this.svg = this.wrapper.append('svg').attr('width',this.width).attr('height',this.height);
	this.init =	function(data){
			 parent.data = data;
			 parent.barWidth = parent.getBarWidth(data.length);
			 parent.makeBaseViz();
			 parent.enterData();
			 parent.addHovers();
	};
	this.createScales = function(){
		parent.xScale = d3.scale.linear().domain([1,parent.data.length -1]).range([parent.padding.left,parent.width -parent.padding.right - parent.barWidth]);
		parent.sizeScale = d3.scale.linear().domain([0,parent.max]).range([parent.height - parent.padding.bottom ,parent.padding.top]);
		parent.reverseSizeScale = d3.scale.linear().domain([0,parent.max]).range([parent.padding.top,parent.height - parent.padding.bottom]);
	};
	this.makeBaseViz = function(){
		var viz = parent.svg;
		parent.main_g = viz.append('g').attr('class','main_g');
		parent.max = d3.max(parent.data,function(d){return parseInt(d["Size"])});
		parent.min = d3.min(parent.data,function(d){return parseInt(d["Size"])});
		parent.label_g = parent.svg.append('g').attr('class','label_g');
		parent.createScales();
		parent.makeLabels();
		d3.select(window).on('resize.one',parent.resize);

	};
	this.makeLabels = function(){
		
		
		parent.yAxis = d3.svg.axis().scale(parent.sizeScale).orient('left').tickSize(-(parent.width-parent.padding.right), 0, 0);
		yAxis_g = parent.label_g.append('g').attr('transform','translate(40,0)').call(parent.yAxis);
		
		parent.label_g.append('text')
			.text('Size (cm)')
			.style('font-size','11px')
			.attr({
				x:function(){ return -( (parent.height/2) - (d3.select(this).node().getBoundingClientRect().width)/2) },
				y:10,
				"text-anchor":'middle',
				transform: "rotate(-90)"
			});
		...
		parent.label_g.append('text')
			.text('Old World Flycatchers Ordered by Size')
			.style('font-size','11px')
			.attr({
				x:function(){ return ( (parent.width/2) - (d3.select(this).node().getBoundingClientRect().width)/2)},
				y: parent.height - 20
			});
			
	};
	this.enterData = function(){
		parent.main_g.selectAll('.bar').data(parent.data).enter()
			.append('rect')
			.attr({
				x: function(d,i){return parent.xScale(i)},
				y: function(d){return  parent.sizeScale(d["Size"])},
				width: parent.barWidth,
				height: function(d){return parent.height - parent.sizeScale(d["Size"]) - parent.padding.bottom},
				'class': function(d){return 'bar ' + d.Species}
			}).style({
				fill: function(d){
					return (Number(d.Cavity) == 1) ? parent.colors.main : parent.colors.secondary;
				},
				cursor: 'pointer'
			});
	};
	this.updateBarLayout = function(){
		parent.main_g.selectAll('.bar')
			.attr({
				x: function(d,i){return parent.xScale(i)},
				y: function(d){return  parent.sizeScale(d["Size"])},
				width: parent.barWidth,
	...
	this.updateCavities = function(){
		parent.main_g.selectAll('.bar').transition().style({
			fill: function(d){
				return (Number(d.Cavity) == 1) ? parent.colors.main : parent.colors.secondary;
			}
		});
	};
	this.updateMigration = function(){
		parent.main_g.selectAll('.bar').transition().style({
			fill: function(d){
				return (Number(d.Mig) == 1) ? parent.colors.main : parent.colors.secondary;
			}
		});
	};
	this.addHovers = function(){
		parent.main_g.selectAll('.bar').on('mouseenter',function(d){
			tooltip.style("visibility", "visible");
			tooltip.text(d.Species.replace('_',' '));
		})
		.on('mouseover',function(){tooltip.style("top", (event.pageY-10)+"px").style("left",(event.pageX+10)+"px");})
		.on('mouseout',function(){tooltip.style("visibility", "hidden");});
	},
	this.resize = function(){
		parent.width = parent.getWidth();
		parent.height = parent.getHeight();
		this.barWidth = parent.getBarWidth(parent.data.length)
		parent.createScales();
		parent.updateBarLayout();
		parent.label_g.selectAll('*').remove();
		parent.makeLabels();			
	};
...
var owf2 = function(wrapper,data){
	var parent = this;
	this.wrapper = d3.select(wrapper);
	this.padding = {
		top: 20,
	...
	this.egg = "M6.198,2.25c0,1.244-2.417,2.25-3.763,2.25c-1.345,0-2.437-1.006-2.437-2.25C-0.002,1.007,1.09,0,2.435,0 C3.782,0,6.198,1.007,6.198,2.25z";
	this.eggWidth = 6.2;
	this.eggHeight= 4.5;
	this.eggMargin = 1;
	this.circleRadius = 10;
	this.circleBorder = 2;
	this.circleHeight = this.circleRadius*2 + this.circleBorder*2;
	this.colors = {
		main: '#fb4770',
		secondary: '#cbc8bb'
	};
	this.getWidth = function(){
		return Math.floor(parseInt(this.wrapper.style('width'))) - Math.floor(parseInt(parent.wrapper.style('padding-left')))  - Math.floor(parseInt(parent.wrapper.style('padding-right'))) ;
	};
	this.width = this.getWidth();
	this.getHeight = function(){
		return (parent.width > 600) ? this.width*.4 : 300;
	...
	this.wrapper.append('button').text('Cavity Nesters').attr('class','switch-cavity').style({
		...
		.on('click',function(){
			parent.wrapper.selectAll('button').style('background-color','rgb(148, 148, 148)');
			parent.positionEggs('cavity');
			d3.select(this).style('background-color',parent.colors.main);
			parent.svg.select('.top-text').text('Cavity Nesters');
			parent.svg.select('.bottom-text').text('Open Nesters');
		});
	this.wrapper.append('button').text('Migratory').attr('class','switch-migration').style({
			'background-color': 'rgb(148, 148, 148)',
			'color': '#fff'
		...
		.on('click',function(){
			parent.wrapper.selectAll('button').style('background-color','rgb(148, 148, 148)');
			parent.positionEggs('migratory');
			d3.select(this).style('background-color',parent.colors.main);
			parent.svg.select('.top-text').text('Migratory');
			parent.svg.select('.bottom-text').text('Non-Migratory');
		});
	this.svg = this.wrapper.append('svg').attr('width',this.width).attr('height',this.height);
	this.init = function(data){
		parent.data = data;
		parent.max = d3.max(parent.data,function(d){return parseInt(d["Clutch"])});
		parent.min = d3.min(parent.data,function(d){return parseInt(d["Clutch"])});
		
		
		// get clutch counts
		parent.cavityClutchCounts = [];
		for (var i = 0; i < parent.max; i++ )
			parent.cavityClutchCounts[i] = 0;
			
		parent.cupClutchCounts = [];
		for (var i = 0; i < parent.max; i++ )
			parent.cupClutchCounts[i] = 0;
			
		// just nonmigratory
		var nonmClutchCounts = [];
		for (var i = 0; i < parent.max; i++ )
			nonmClutchCounts[i] = 0;
			
		for (var c in data) {
			if (data[c].Cavity == 1) {
				parent.cavityClutchCounts[data[c].Clutch - 1]++;
			} else {
				parent.cupClutchCounts[data[c].Clutch - 1]++;
			}
		}
		
		for (var c in data) {
			if (data[c].Mig == 0) {
				nonmClutchCounts[data[c].Clutch - 1]++;
			}
		}
		
		parent.maxCavityCount = d3.max(parent.cavityClutchCounts);
		parent.maxCupCount = d3.max(parent.cupClutchCounts);
		parent.maxNonmCount = d3.max(nonmClutchCounts);
		
		parent.cavityHeight = parent.maxCavityCount * parent.eggHeight + parent.maxCavityCount * parent.eggMargin;
		parent.cupHeight = parent.maxCupCount * parent.eggHeight + parent.maxCupCount * parent.eggMargin;
		parent.nonmHeight = parent.maxNonmCount * parent.eggHeight + parent.maxCupCount * parent.eggMargin;
		
		parent.height = parent.cavityHeight + parent.nonmHeight + parent.circleHeight + 4 + parent.padding.top + parent.padding.bottom + 20;
		parent.svg.attr('height',parent.height);
		
		parent.createScales();
		parent.makeBaseViz();
	};
	this.createScales = function(){
		parent.xScale = d3.scale.linear().domain([1,parent.max]).range([parent.padding.left,parent.width -parent.padding.right]);
	};
	this.makeBaseViz = function(){
		parent.main_g = parent.svg.append('g').attr('class','main_g');
		
		parent.symbol = parent.svg.append('symbol').attr({
			viewBox: "0 0 6.198 4.5",
			id: "egg"
		});
		...
		parent.makeLabels();
		parent.positionEggs('cavity');
		d3.select(window).on('resize.two',parent.resize)
	};
	this.makeAxis = function(){
		parent.axis = parent.svg.append('g').attr('class','egg-axis').attr('transform','translate(0, ' + (parent.padding.top + parent.cavityHeight) + ')');
		
		parent.axis.append('line')
			.style({
				stroke: '#949494',
				"stroke-width": '3px'
			})
			.attr({
				x1: parent.padding.left,
				y1: 0,
				x2: parent.width - parent.padding.right,
				y2: 0
			});
			
		for (var i = 1; i <= parent.max; i++){
			parent.axis.append('circle')
				.attr({
					r: parent.circleRadius,
					cx: parent.xScale(i),
					cy:0
				}).style({
					fill: 'white',
			...
			parent.axis.append('text')
				.text(i)
				.attr({
					x: function(){return parent.xScale(i)},
					y: function(){return parent.circleRadius - d3.select(this).node().getBoundingClientRect().height/2 + 2}
				}).style({
					"font-size":'9.5px',
					"font-weight":'bold',
					"text-anchor":'middle'
	...
	this.makeLabels = function(){
		parent.svg.append('text')
			...
			.text('MAX CLUTCH SIZE')
			.call(wrap,40);
			
		parent.svg.append('text').attr('class','top-text').text('Cavity Nesters')
			.style({
				'font-size':'10px',
				'text-anchor':'middle',
				fill: parent.colors.main
			})
			.attr({
				x: function(){return -(parent.cavityHeight/2 + parent.padding.top + d3.select(this).node().getBoundingClientRect().height/2 )},
				y: 40,
				transform: "rotate(-90)"
			});
		parent.svg.append('text').attr('class','bottom-text').text('Open Nesters')
			.style({
				'font-size':'10px',
				'text-anchor':'middle',
				fill: parent.colors.secondary
			})
			.attr({
				x: function(){return -(parent.cavityHeight + parent.padding.top + d3.select(this).node().getBoundingClientRect().height/2 + parent.nonmHeight/2 )},
				y: 40,
				transform: "rotate(-90)"
			});
	};
	this.initEggs = false;
	this.flavor == 'cavity';
	this.positionEggs = function(flavor){
		...
		for (var c in parent.clutchCounts) {
			for (var i = 0; i < parent.max; i++){
				parent.clutchCounts[c][i] = 0;
			}
		}
		...
		if (!parent.initEggs){
			parent.enterEggs();
			parent.addHovers();
			parent.initEggs = true;
		}
		parent.updateEggs(flavor);
		
		parent.flavor = flavor;
				
	};
	this.enterEggs = function(){
		parent.main_g.selectAll('.eggs').data(parent.data).enter().append('g').attr('class','eggs')
			.attr({
				x:0,
				y:0,
			...
			.style({
				fill: function(d){return d.Cavity == 1 ? parent.colors.main : parent.colors.secondary; }
			});
			
	};
	this.updateEggs = function(flavor){
		parent.main_g.selectAll('.eggs').transition()
			.attr({
				"transform": function(d){
					var cue = flavor == 'cavity' ? d.Cavity : d.Mig;
					var y;
					if (flavor == 'cavity') {
						
						if (d.Cavity == 1) {
							y = parent.cavityHeight - (parent.clutchCounts.cavity[d.Clutch - 1] * (parent.eggHeight + parent.eggMargin));
							parent.clutchCounts.cavity[d.Clutch - 1]++;
						} else {
							y = parent.cavityHeight + parent.circleRadius + 25 + (parent.clutchCounts.cup[d.Clutch - 1] * (parent.eggHeight + parent.eggMargin));
							parent.clutchCounts.cup[d.Clutch - 1]++;
						}
						
					} else if (flavor == 'migratory') {
						
						if (d.Mig == 1) {
							y = parent.cavityHeight - (parent.clutchCounts.migratory[d.Clutch - 1] * (parent.eggHeight + parent.eggMargin));
							parent.clutchCounts.migratory[d.Clutch - 1]++;
						} else {
							y = parent.cavityHeight + parent.circleRadius + 25 + (parent.clutchCounts.nonmigratory[d.Clutch - 1] * (parent.eggHeight + parent.eggMargin));
							parent.clutchCounts.nonmigratory[d.Clutch - 1]++;
						}
						
					}
					
					return 'translate(' +
						(parent.xScale(d.Clutch) - 2) +
						',' +
						y +
						')';
				}
			...
			.select('path').style({
				fill: function(d){					
					if (flavor == 'cavity') {
						return d.Cavity == 1 ? parent.colors.main : parent.colors.secondary;
					} else if (flavor == 'migratory') {
						return d.Mig == 1 ? parent.colors.main : parent.colors.secondary;
					}
	...
	this.addHovers = function(){
		parent.main_g.selectAll('.eggs').on('mouseenter',function(d){
			tooltip.style("visibility", "visible");
			tooltip.text(d.Species.replace('_',' '));
		})
		.on('mouseover',function(){tooltip.style("top", (event.pageY-10)+"px").style("left",(event.pageX+10)+"px");})
	...
	this.makeCavityEggs = function(){
		var g;
		for (var i =0; i < parent.cavityClutchCounts.length; i++) {
			g = parent.main_g.append('g').attr('class','egg-wrapper-cavity');
			
			for (var j=0; j < parent.cavityClutchCounts[i]; j++) {
				g.append('path').attr({
					d: parent.egg,
					transform: 'translate(0,'+ (j * (parent.eggHeight + parent.eggMargin)) +')'
				}).style('fill','#fb4770');
			}
			
			
			g.attr('transform','translate('+ (parent.xScale(i+1) - (parent.circleRadius/2) +2) + ',' + (parent.cavityHeight - g.node().getBoundingClientRect().height) +')');
		}
	};
	this.makeCupEggs = function(){
		var g;
		for (var i =0; i < parent.cupClutchCounts.length; i++) {
			g = parent.main_g.append('g').attr('class','egg-wrapper-cup');
			
			for (var j=0; j < parent.cupClutchCounts[i]; j++) {
				g.append('path').attr({
					d: parent.egg,
					transform: 'translate(0,'+ (j * (parent.eggHeight + parent.eggMargin)) +')'
				}).style('fill','#cbc8bb');
			...
			g.attr('transform','translate('+ (parent.xScale(i+1) - (parent.circleRadius/2) +2) + ',' + (parent.cavityHeight + parent.circleRadius + 30) +')');
		}
	};
	this.resize = function(){
		parent.width = parent.getWidth();
		parent.svg.attr('width',parent.width);
		parent.createScales();
		parent.axis.selectAll("*").remove();
		parent.makeAxis();
		parent.positionEggs(parent.flavor);
	};
...
d3.csv('/citsci/wp-content/themes/CitSciMonthlyv2.1/js/monthly/2015-03-nests/owf.csv',function(d){
	var viz = new owf('#cav-wrapper',d);
	var viz2 = new owf2('#egg-wrapper',d);
});
...
module.exports = function(data, config) {
  ...
  // geometry
  config.width = config.width || parseInt(d3.select('#circle-wrapper').style('width')) - 20;
  config.height = config.height || 500;
  config.margin = config.margin || 50;
  ...
  config.targetPadding = config.targetPadding || 10;
  ...
  config.mylayout.sortSubgroups = config.mylayout.sortSubgroups || d3.ascending;
  config.mylayout.sortChords = config.mylayout.sortChords || null;
  config.mylayout.threshold = config.mylayout.threshold || 1;
  config.mylayout.labelThreshold = config.mylayout.labelThreshold || 1;

  config.maxRegionsOpen = config.maxRegionsOpen || 2;
  config.infoPopupDelay = config.infoPopupDelay || 300;

  //var colors = d3.scale.linear().range(['#FB4771','#FF8648','#00D891','#59F000']).domain(data.regions)
  var colors = d3.scale.linear().range(['#FB4771','#CBC8BB','#CBC29D','#FB86A1']).domain(data.regions)
...
*/
  ...
  var textPathArc2 = d3.svg.arc()
      .innerRadius(config.outerRadius + 20)
 ...
 var b2 = d3.select('#circle-wrapper').append('button').attr('id','all-btn').text('All State Changes').style('background-color','#fb4770');
 var b1 = d3.select('#circle-wrapper').append('button').attr('id','story-btn').text('State Changes To Migratory Only').style('background-color','rgb(148, 148, 148)');
...
*/
    ...
    groupTextPathPath
      ...
      .attrTween("d", function(d) {
        ...
        if (d.region == 4) {
	        var n = previous.groups[d.id];
	        n.startAngle = 4.5;
	        n.endAngle = 4;
	        i = d3.interpolate(n,n);
	        return function (t) {
    ...
    chord.enter()
      .append("path")
      .attr("class", "chord")
      .attr('id',function(d){return 'chord-' + d.source.id + '-' + d.target.id;})
...
/*
   	  var spine = function(d){
	   var s_a = d.source.midAngle-Math.PI / 2, //the -Math.PI/2 came from looking at the chord code in d3
	       t_a = d.target.midAngle-Math.PI / 2; //the -Math.PI/2 came from looking at the chord code in d3
	                        
	   var s_p = [ r0 * Math.cos(s_a), r0 * Math.sin(s_a) ],
	       t_p = [ r0 * Math.cos(t_a), r0 * Math.sin(t_a) ];
	                        
	   return  "M" + s_p + "Q 0,0 " + t_p;
	};
    
  var cds = d3.selectAll('.chord');
    
   for (i=0;i<cds[0].length;i++){
	   d3.select(cds[0][i]).append('path')
	      		.style('stroke','black')
	      		.attr('d',function(d){console.log(d); return spine(d)});
   }
...
*/
	...
	b1.on('click',function(){
		d3.selectAll('path.chord').each(function(d){
			...
			var id = that.attr('id');
			that.classed('fade',false);	
			if (id!=='chord-2-6' && id!== 'chord-0-6' && id!=='chord-0-4') {
				that.classed('fade',true);	
			}
		});
		b1.style('background-color','#fb4770');
		b2.style('background-color','rgb(148, 148, 148)');
		btnState = 'mig';
	});
	...
	b2.on('click',function(){
		...
		b2.style('background-color','#fb4770');
		b1.style('background-color','rgb(148, 148, 148)');
		btnState = 'all';
	});
...
/*
     d3.selectAll('#chord-0-6,#chord-2-0').on('mousenter',function(){
	     console.log('jere');
     })  ;  
*/ 
...
module.exports = function() {
  ...
  function chord(d, i) {
    ...
    var ccp = cubic_control_points(s, t, s.r * .618);
...
(1)
 ...
 CircularMigrationPlot({
      data: '/citsci/wp-content/themes/CitSciMonthlyv2.1/js/monthly/2015-03-nests/owf-flow.json',
